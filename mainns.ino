...
const char* serverUrl = "http://192.168.8.13/PROJECT_1/welcome.php";
int fingerprintId = 0;

const char* ssid = "jairus"; // Replace with your WiFi SSID
const char* password = "12345678"; // Replace with your WiFi password

LiquidCrystal_I2C lcd(0x3F, 16, 2); // Set the LCD address and dimensions

// Define RX and TX pins for Serial2
#define RX_PIN 16 // GPIO pin number for RX
#define TX_PIN 17 // GPIO pin number for TX
...
void setup() {
  ...
  // Initialize AsyncWebServer if needed
  //server.on("/", HTTP_POST, handlePostRequest);   // Define POST request handler
  //server.begin();

  // Set the data rate for the sensor serial port with explicit RX and TX pin assignment
  mySerial.begin(57600, SERIAL_8N1, RX_PIN, TX_PIN);
  delay(10); // Extra delay after serial begin for stability
  ...
  if (finger.verifyPassword()) {
    ...
    Serial.println("Fingerprint sensor found!"); // Debug output
  } else {
    lcd.clear();
    lcd.print("Fingerprint");
    lcd.setCursor(0, 1);
    lcd.print("sensor not found!");
    Serial.println("Fingerprint sensor not found!"); // Debug output
    while (1) { delay(1); }
  }

  delay(500); // Delay after sensor found message
  ...
  // Debug output for sensor parameters
